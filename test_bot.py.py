...
def echo_all(message):
	if message.text == 'Привет':
		...
		bot.send_message(message.from_user.id, 'Привет! Давай познакомимся!\'\nКак тебя зовут?')
...
def callback_worker(call):
	if call.data == 'yes':
		bot.send_message(call.message.chat.id, 'Приятно познакомиться! Я запомню..')
	elif call.data == 'no':
		bot.send_message(call.message.chat.id, 'Попробуем еще раз?')
		bot.send_message(call.message.chat.id, 'Привет! Давай познакомимся!\'\nКак тебя зовут?')
